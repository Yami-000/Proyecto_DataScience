...
DEFAULT_TARGET_NAME = "Vanitas"
DEFAULT_TARGET_TAG = "DANTE"
...
def flatten_allied_team(match: Dict[str, Any], target_name: str, target_tag: Optional[str]) -> List[Dict[str, Any]]:
    ...
    for participant in info.get('participants', []) or []:
        ...
        row = {
            ...
            'gold_per_min': participant.get('challenges', {}).get('goldPerMinute') or np.nan,
            'vision_score_per_min': participant.get('challenges', {}).get('visionScorePerMinute') or np.nan,
            'kda': participant.get('challenges', {}).get('kda') or np.nan,
        }
        rows.append(row)
...
def clean_player_df(df: pd.DataFrame) -> pd.DataFrame:
    ...
    fill_median = ['gold_per_min', 'vision_score_per_min', 'kda']
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument('--target-name', type=str, default=DEFAULT_TARGET_NAME, help='Riot ID (nombre) del jugador objetivo')
    parser.add_argument('--target-tag', type=str, default=DEFAULT_TARGET_TAG, help='Riot ID Tagline del jugador objetivo')
