...
def get_league_players():
    try:
        league_data = lol_watcher.league.entries(region, SETTINGS["QUEUE"], SETTINGS["TIER"], SETTINGS["DIVISION"])
...
def get_matches_data(puuid):
    ...
    try:
        ...
        match_ids = lol_watcher.match.matchlist_by_puuid(region, puuid, count=SETTINGS["MATCHES_PER_PLAYER"])
